...
class Blog extends CI_Controller{
	...
	function kategori(){
		...
		if($query->num_rows() > 0){
			...
			$x['category']=$this->db->get('tbl_kategori');
			$x['populer']=$this->db->query("SELECT * FROM tbl_tulisan ORDER BY tulisan_views DESC LIMIT 5");
			$this->session->unset_userdata('msg_error_search');
			$this->load->view('depan/v_blog',$x);
		}else{
			echo $this->session->set_flashdata('msg_error_search','<div class="alert alert-danger" role="alert"><button type="button" class="close" data-dismiss="alert"><span class="fa fa-close"></span></button> Tidak ada artikel unruk kategori <b>'.$kategori.'</b></div>');
			redirect('artikel');
		}
    ...
    function search(){
        ...
        $query=$this->m_tulisan->cari_berita($keyword);
          ...
          $this->load->view('depan/v_blog',$x);
				 ...
				 echo $this->session->set_flashdata('msg_error_search','<div class="alert alert-danger" role="alert"><button type="button" class="close" data-dismiss="alert"><span class="fa fa-close"></span></button> Tidak dapat menemukan artikel dengan kata kunci <b>'.$keyword.'</b></div>');
				 redirect('artikel');
			 }
    }
